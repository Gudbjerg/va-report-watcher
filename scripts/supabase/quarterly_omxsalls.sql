-- Recreate quarterly table for OMXSALLS
-- Safe to re-run: drops then creates with required columns and indexes

BEGIN;

DROP TABLE IF EXISTS public.index_quarterly_omxsalls CASCADE;

CREATE TABLE public.index_quarterly_omxsalls (
...
CREATE INDEX IF NOT EXISTS idx_q_omxsalls_as_of ON public.index_quarterly_omxsalls(as_of DESC);
CREATE INDEX IF NOT EXISTS idx_q_omxsalls_mcap_uncapped ON public.index_quarterly_omxsalls(mcap_uncapped DESC);
