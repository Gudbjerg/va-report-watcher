-- Recreate quarterly table for KAXCAP
-- Safe to re-run: drops then creates with required columns and indexes

BEGIN;

DROP TABLE IF EXISTS public.index_quarterly_kaxcap CASCADE;

CREATE TABLE public.index_quarterly_kaxcap (
...
CREATE INDEX IF NOT EXISTS idx_q_kaxcap_as_of ON public.index_quarterly_kaxcap(as_of DESC);
CREATE INDEX IF NOT EXISTS idx_q_kaxcap_mcap_uncapped ON public.index_quarterly_kaxcap(mcap_uncapped DESC);
